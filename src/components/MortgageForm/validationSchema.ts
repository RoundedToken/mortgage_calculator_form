...
import { TFormValues } from './initialValues';
import { requiredMessage } from './constants';

export const validationSchema = Yup.object<TFormValues>({
    cost: Yup.string()
        .test(
    ...
    time: Yup.string()
        .test('isMoreOrEquals4', 'Срок ипотеки не может быть меньше 4 лет', v => +removeCommas(v || '') >= 4)
        .test('isLessOrEquals30', 'Срок ипотеки не может превышать 30 лет', v => +removeCommas(v || '') <= 30)
        .required(requiredMessage),
